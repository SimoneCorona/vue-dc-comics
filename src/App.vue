<template>
  <div id="app">
    <!-- Header -->
    <MyAppHeader />
    <!-- Main Jumbotron + Main content -->
    <main>
        <MyAppMainBanner />
        <Span class="blue-btn first"><a>CURRENT SERIES</a></Span>
        <div class="container">
          <MyAppMain v-for="(item, index) in product" :key= "index" :thumb="item.thumb" :series="item.series"/>
        </div>
        <Span class="blue-btn second"><a>LOAD MORE</a></Span>
    </main> 
    <!-- 2° Jumbotron + Footer -->
    <MyAppJumbotron />
...
import MyAppJumbotron from "./components/MyAppJumbotron.vue";
import MyAppMain from "./components/MyAppMain.vue";
import MyAppMainBanner from "./components/MyAppMainBanner.vue";
